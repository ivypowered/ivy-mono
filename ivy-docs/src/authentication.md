...
<small class="route-tag">GET</small> `/verify?game={game}&message={message}&signature={signature}` Verify that the given `message` is valid for the game `game`, and correctly signed by `signature`.
