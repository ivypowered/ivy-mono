...
static void game_withdraw_claim(
    ...
    // Idempotency: ensure we haven't done this withdraw before
    if (account_exists(&accounts->withdraw)) {
        // We're good, withdraw already processed
        return;
    }
...
static void game_deposit_complete(
    ...
    // Idempotency: ensure we haven't done this deposit before
    if (account_exists(&accounts->deposit)) {
        // we're good
        // (or there's a collision... not our fault!)
        return;
    }
