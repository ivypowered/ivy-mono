 ...
 * payment.php
 *
 * Handles game payment (deposit/burn) processing and confirmation
...
// Process payment logic in an anonymous function
$result = (function () use ($variant) {
    ...
    try {
        ...
        if ($payment_info !== null) {
            ...
            header(
                "Location: /{$variant}-complete?id=" .
                    urlencode($payment_id) .
                    "&signature=" .
                    urlencode($payment_info["signature"]) .
                    "&game=" .
        ...
        // Continue if payment check fails - we'll try to create it
    }

    // Extract payment amount from ID
    try {
        $id_bytes = hex2bin($payment_id);
        if (strlen($id_bytes) !== 32) {
            return ["error" => "Invalid {$variant} ID length"];
        }
        ...
        return ["error" => "Invalid {$variant} ID format: " . $e->getMessage()];
    }
    ...
    try {
        $payment_data = [
            ...
            "{$variant}_id" => $payment_id,
        ];

        $payment_response = call_backend(
            "/tx/game/{$variant}-complete",
            "POST",
            $payment_data
        );
        if ($payment_response === null) {
            throw new Exception("Failed to prepare {$variant} transaction");
        }

        $transaction_data = [
            "tx" => $payment_response,
            "returnUrl" => "/{$variant}-complete",
            "returnParams" => [
                "id" => $payment_id,
                "game" => $game_public_key,
            ],
        ...
        return [
            "error" =>
                ucfirst($variant) . " preparation failed: " . $e->getMessage(),
        ];
    }
...
$formatted_payment_amount = empty($error_message)
    ? number_format($payment_amount / 1000000000, 2)
    : "0.00";

$variant_display = ucfirst($variant);

$title = "ivy | $variant";
$description = "Complete your $variant action on Ivy";
...
<main class="py-8">
    <div class="mx-auto max-w-3xl px-6">
        <!-- Confirmation Page -->
        <h1 class="text-3xl font-bold mb-8 text-center">Confirm <?= $variant_display ?></h1>
        ...
        <div class="mb-6 p-4 border-2 border-red-400 bg-red-950/20">
            ...
            <div class="mt-4 text-center">
                ...
                <p class="text-red-300 mb-3"><?= $variant_display ?> requires a connected wallet address.</p>
        ...
        <div class="border-2 border-emerald-400 bg-zinc-900">
            ...
            <div class="p-4 flex flex-col gap-4">
                ...
                <!-- Payment Amount -->
                <div class="mt-2">
                    <div class="text-xs uppercase text-zinc-500 mb-1"><?= $variant_display ?> Amount</div>
                    <div class="bg-zinc-800 p-3 border border-zinc-700 flex justify-between items-center">
                        <span class="text-xl text-emerald-400 font-bold"><?= $formatted_payment_amount ?></span>
                ...
                <!-- Payment ID -->
                <div>
                    <div class="text-xs uppercase text-zinc-500 mb-1"><?= $variant_display ?> ID</div>
                    <div class="font-mono text-sm text-zinc-300 bg-zinc-800 p-2 border border-zinc-700 overflow-x-auto">
